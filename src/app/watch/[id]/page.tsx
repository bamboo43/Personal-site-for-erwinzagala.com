...
export default async function WatchVideoPage({ params }: PageProps) {
  ...
  return (
    <Container width="wide" className="py-10 sm:py-14">
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-stone-200/80 bg-stone-900 shadow-sm dark:border-stone-800">
      ...
      <header className="mt-8 max-w-3xl">
        ...
        <h1 className="mt-3 font-serif text-3xl font-semibold leading-tight tracking-tight text-stone-900 sm:text-[2.1rem] dark:text-stone-50">
          {video.title}
        </h1>
      ...
      {related.length > 0 ? (
        <section className="mt-14 border-t border-stone-200/80 pt-10 dark:border-stone-800">
          ...
          <Link
            href="/watch"
            className="mt-8 inline-block text-sm font-medium text-teal-800 hover:underline dark:text-teal-300"
          >
            All videos →
          </Link>
        </section>
